...
class Toys extends Component {
    ...
    nameAscending=()=>{
        Axios.get(`${API_URL}/products?q=${this.state.search}&kategoriId=3&_sort=name&_order=asc`)
        .then((resNameAsc)=>{
    ...
    nameDescending=()=>{
        Axios.get(`${API_URL}/products?q=${this.state.search}&kategoriId=3&_sort=name&_order=desc`)
        .then((resNameDesc)=>{
    ...
    priceAscending=()=>{
        Axios.get(`${API_URL}/products?q=${this.state.search}&kategoriId=3&_sort=harga&_order=asc`)
        .then((resNameAsc)=>{
            this.setState({products:resNameAsc.data})
        }).catch((err)=>{
            console.log(err)
    ...
    priceDescending=()=>{
        Axios.get(`${API_URL}/products?q=${this.state.search}&kategoriId=3&_sort=harga&_order=desc`)
        .then((resNameDesc)=>{
            this.setState({products:resNameDesc.data})
        }).catch((err)=>{
            console.log(err)
    ...
    searchOnChange=(e)=>{
        ...
        Axios.get(`${API_URL}/products?kategoriId=3&name_like=${this.state.search}`)
        .then((resSearch)=>{
        ...
        return ( 
            <div>
                <div className='allproduct-container'>
                    ...
                    <h2 className='allproduct-header'>Toys</h2>
                        <div className='row'>
                        {
